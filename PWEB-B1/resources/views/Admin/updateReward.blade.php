...
<div class="p-6 bg-gray-100 min-h-screen">
    <div class="bg-white rounded-xl shadow-md p-6 max-w-3xl mx-auto">
        <h2 class="text-2xl font-semibold mb-6 text-gray-800">Edit Reward</h2>
        ...
        <form action="{{ route('reward.update', $reward->reward_id) }}" method="POST" enctype="multipart/form-data">
            @csrf

            <!-- Nama -->
            <div class="mb-4">
                <label for="judul" class="block text-sm font-medium text-gray-700 mb-1">Nama</label>
                <input type="text" id="nama" name="nama" value="{{ old('nama', $reward->nama) }}"
            ...
            <div class="mb-4">
               ...
               <!-- Preview gambar lama -->
                @if($reward->gambar)
                    <img id="preview-lama" src="{{ asset('img/' . $reward->gambar) }}" alt="Gambar Artikel"
            ...
            <!-- Konten -->
            <div class="mb-4">
                <label for="konten" class="block text-sm font-medium text-gray-700 mb-1">Jumalah Ember Harga</label>
                <textarea id="jumlah_ember_harga" name="jumlah_ember_harga" rows="6"
                          class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-green-500"
                          required>{{ old('jumlah_ember_harga', $reward->jumlah_ember_harga) }}</textarea>
            </div>
